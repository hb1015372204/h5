  ...
  <div class="container">
    ...
    <div class="carInfo">
      ...
      <div class="boxst_s" :class="isSticty?'isSticty':''">
        <ul class="box-sticky">
          <li v-for="(item, index) in typeList" :key="index" :class="isactive==index?'active':''" @click="handleTitle(item, index)" :data-to="index">{{item.name}}</li>
        </ul>
      </div>
    
      <div class="box-hd mt18 tcenter" >
...
export default {
  ...
  methods:{
    ...
    handleTitle(item, el) {
      ...
      TDAPP.onEvent (`导航-${item.name}`, '', {'导航': item.name});
    },
    handleScroll () {
